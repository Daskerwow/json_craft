...
//
// ─── Naming convention (applies to every parser in this library) ──────────
...
//                only where an incorrect value is a genuine bug, not bad
...
///
/// Accepted input: `null` → `null`; `String` → itself (trimmed unless
/// [trim] is `false`); `num`/`bool` → their `.toString()`. Anything else
/// (List, Map, custom objects) → `null` — deliberate, so an accidental
/// `Object` doesn't silently turn into `"Instance of 'Foo'"`.
@pragma('vm:prefer-inline')
String? stringOrNull(Object? v, {bool trim = true}) => switch (v) {
  null => null,
  final String s => trim ? s.trim() : s,
...
@pragma('vm:prefer-inline')
String stringOrEmpty(Object? v, {bool trim = true}) =>
    stringOrNull(v, trim: trim) ?? '';

@pragma('vm:prefer-inline')
ParserFn<String> stringOrDefault(String fallback, {bool trim = true}) =>
    (Object? v) => stringOrNull(v, trim: trim) ?? fallback;

@pragma('vm:prefer-inline')
String stringOrThrow(Object? v, {bool trim = true}) =>
    stringOrNull(v, trim: trim) ??
...
@pragma('vm:prefer-inline')
ParserFn<int> intOrDefault(int fallback) =>
...
@pragma('vm:prefer-inline')
ParserFn<double> doubleOrDefault(double fallback) =>
...
@pragma('vm:prefer-inline')
ParserFn<num> numOrDefault(num fallback) =>
...
@pragma('vm:prefer-inline')
ParserFn<BigInt> bigIntOrDefault(BigInt fallback) =>
...
///
/// Returns `null` for relative URIs, `mailto:`, `urn:`, etc. — use instead
/// of [uriOrNull] when the field must hold a real web address.
@pragma('vm:prefer-inline')
...
@pragma('vm:prefer-inline')
ParserFn<Uri> uriOrDefault(Uri fallback) =>
